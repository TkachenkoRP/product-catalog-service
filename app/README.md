...
Это основной модуль приложения, использующий модули `my-audit` и `my-logger` для расширенного логирования и аудита.
...
Приложение автоматически логирует все действия контроллеров:
...
```yaml
audition:
  enabled: true  # Включение/отключение аудита
  ```
...
3. Запустите `docker-compose -f ../docker/docker-compose.yml up -d` в директории проекта.
4. Запустите приложение `mvn clean spring-boot:run`.
...
- Аудит: `../logs/audit.log` (действия пользователей)
- Детальные логи: `../logs/detailed.log` (бизнес-логика)